...
const PROJECT_ID = 'smart-city-hub-573d1';

// Get access token from Firebase CLI
let TOKEN;
try {
  TOKEN = execSync('firebase auth:print-access-token 2>nul', { encoding: 'utf8' }).trim();
} catch(e) {
  // Try gcloud
  try {
...
const now = Date.now();
const WARD_COORDS = {
  'Chennai - Ward 1':  [13.0604, 80.2496],
  'Chennai - Ward 2':  [13.0650, 80.2550],
  'Chennai - Ward 3':  [13.0700, 80.2600],
  'Chennai - Ward 4':  [13.0750, 80.2650],
  'Chennai - Ward 5':  [13.0130, 80.2108],
  'Chennai - Ward 6':  [13.0180, 80.2150],
  'Chennai - Ward 7':  [13.0694, 80.1948],
  'Chennai - Ward 8':  [13.0740, 80.1990],
  'Chennai - Ward 9':  [13.0524, 80.2341],
  'Chennai - Ward 10': [13.0500, 80.2824],
  'Chennai - Ward 11': [13.0550, 80.2870],
  'Chennai - Ward 12': [13.0569, 80.2425],
  'Chennai - Ward 13': [13.1143, 80.1548],
  'Chennai - Ward 14': [13.0012, 80.2565],
  'Chennai - Ward 15': [12.9815, 80.2180],
  'Coimbatore - Ward 1': [11.0168, 76.9558],
  'Coimbatore - Ward 2': [11.0010, 76.9674],
  'Coimbatore - Ward 3': [11.0200, 76.9700],
  'Madurai - Ward 1':  [9.9252, 78.1198],
  'Madurai - Ward 2':  [9.9195, 78.1193],
  'Madurai - Ward 3':  [9.9300, 78.1250],
  'Trichy - Ward 1':   [10.8050, 78.6856],
  'Trichy - Ward 2':   [10.8100, 78.6900],
  'Salem - Ward 1':    [11.6643, 78.1460],
  'Salem - Ward 2':    [11.6700, 78.1500],
  'Salem - Ward 3':    [11.6580, 78.1420],
  'Salem - Ward 4':    [11.6620, 78.1480],
};

const j = (v) => parseFloat((v + (Math.random()-0.5)*0.003).toFixed(6));
const ts = (hrs) => ({ _seconds: Math.floor((now - hrs*3600000)/1000), _nanoseconds: 0 });
...
function firestoreDoc(c) {
  const coords = WARD_COORDS[c.ward] || [13.0604, 80.2496];
  const secs = Math.floor((now - c.hrs * 3600000) / 1000);
  return {
    fields: {
      title:           { stringValue: c.title },
      description:     { stringValue: `${c.title} reported in ${c.ward}. Immediate attention required.` },
      category:        { stringValue: c.category },
      ward:            { stringValue: c.ward },
      status:          { stringValue: c.status },
      latitude:        { doubleValue: j(coords[0]) },
      longitude:       { doubleValue: j(coords[1]) },
      userId:          { stringValue: 'seed-user' },
      upvotes:         { integerValue: String(Math.floor(Math.random()*60)+10) },
      priorityScore:   { integerValue: '0' },
      isHotspot:       { booleanValue: false },
      timestamp:       { timestampValue: new Date(now - c.hrs*3600000).toISOString() },
      createdAt:       { timestampValue: new Date(now - c.hrs*3600000).toISOString() },
    }
  };
}
...
async function seed() {
  console.log('🌱 Seeding 100 complaints via Firestore REST API...');
  let ok = 0;
  for (const c of COMPLAINTS) {
    const path = `/v1/projects/${PROJECT_ID}/databases/(default)/documents/complaints/${c.id}`;
    const res = await request('PATCH', path, firestoreDoc(c));
    if (res.status === 200) { ok++; process.stdout.write(`\r✅ ${ok}/100`); }
    else console.error(`\n❌ ${c.id}: ${res.status}`, res.body.error?.message);
  }

  // Seed city metrics
  await request('PATCH',
    `/v1/projects/${PROJECT_ID}/databases/(default)/documents/cityMetrics/current`,
    { fields: {
      complaintsCount: { integerValue: '100' },
      pendingCount:    { integerValue: '85' },
      resolvedCount:   { integerValue: '7' },
      pollution:       { integerValue: '68' },
      traffic:         { integerValue: '72' },
      cityHealthScore: { integerValue: '28' },
      healthGrade:     { stringValue: 'D' },
      updatedAt:       { stringValue: new Date().toISOString() }
    }}
  );

  console.log(`\n🎉 Done! ${ok} complaints seeded to Firestore`);
}
